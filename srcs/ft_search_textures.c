...
/*   Updated: 2021/02/28 14:59:19 by barodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			search_textures_path_2(char *direction, int i, char *buf, t_conf *conf)
{
		if (!ft_strncmp("WE", direction, 2))
	{
		if (ft_strlen(conf->NO_path))
			return (-1);
		conf->WE_path = ft_substr(buf, i, ft_strlen(buf) - i);
	}
	if (!ft_strncmp("EA", direction, 2))
	{
		if (ft_strlen(conf->SO_path))
			return (-1);
		conf->EA_path = ft_substr(buf, i, ft_strlen(buf) - i);
	}
		if (!ft_strncmp("S ", direction, 2))
	{
		if (ft_strlen(conf->NO_path))
			return (-1);
		conf->S_path = ft_substr(buf, i, ft_strlen(buf) - i);
	}
	return (1);
}
...
int			search_textures_path(t_conf *conf, char *buf)
{
	...
	if (!ft_strncmp("NO", direction, 2))
	{
		if (ft_strlen(conf->NO_path))
			return (-1);
		conf->NO_path = ft_substr(buf, i, ft_strlen(buf) - i);
	}
	if (!ft_strncmp("SO", direction, 2))
	{
		if (ft_strlen(conf->SO_path))
			return (-1);
		conf->SO_path = ft_substr(buf, i, ft_strlen(buf) - i);
	}
