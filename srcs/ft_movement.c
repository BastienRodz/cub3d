...
/*   Updated: 2021/03/15 13:45:54 by barodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_is_collision(t_conf *conf)
{
	...
	movestep = conf->p.walkdir * conf->p.movespeed;
	x = (int)floorf(conf->p.x);
	y = (int)floorf(conf->p.y);
	if (is_in_set(conf->m.map[y][x], "12") && (conf->p.walkdir == 1
		|| conf->p.walkdir == -1))
	{
		conf->p.x -= cos(conf->p.angle) * movestep;
		conf->p.y -= sin(conf->p.angle) * movestep;
	}
		if (is_in_set(conf->m.map[y][x], "12") && conf->p.sidewalk == 1)
	{
		conf->p.x -= cos(conf->p.angle + (90 * (3.1415 / 180))) * conf->p.movespeed;
		conf->p.y -= sin(conf->p.angle + (90 * (3.1415 / 180))) * conf->p.movespeed;
	}
		if (is_in_set(conf->m.map[y][x], "12") && conf->p.sidewalk == -1)
	{
		conf->p.x -= cos(conf->p.angle - (90 * (3.1415 / 180))) * conf->p.movespeed;
